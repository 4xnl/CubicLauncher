...
beforeEach(() => {
	...
	invoke = mock((command, args) => {
		if (command !== "start_instance_server_ping")
			return Promise.resolve({ revision: "updated", servers: [] });
		return new Promise((resolve, reject) =>
			starts.push({ args, resolve, reject }),
		);
	});
...
test("cancels native work even when the component closes before start returns", async () => {
	const status = mock();
	const error = mock();
	const cancel = pingInstanceServers(
		"instance-A",
		[{ index: 0, address: "localhost" }],
		status,
		error,
	);
	cancel();
	cancel();
	starts[0].args.onStatus.onmessage({
		indices: [0],
		status: { online: true },
		done: false,
	});
	...
	expect(
		invoke.mock.calls.filter(
			([command]) => command === "cancel_instance_server_ping",
		),
	).toEqual([
...
test("switching instances rejects stale results and cancels only their native request", async () => {
	const seen = [];
	const cancelA = pingInstanceServers(
		"A",
		[{ index: 0, address: "a.example" }],
		(event) => seen.push(["A", event]),
		mock(),
	);
	cancelA();
	const cancelB = pingInstanceServers(
		"B",
		[{ index: 0, address: "b.example" }],
		(event) => seen.push(["B", event]),
		mock(),
	);
	...
	expect(
		invoke.mock.calls
			.filter(([command]) => command === "cancel_instance_server_ping")
...
test("reports completion and read conflicts, but ignores failures after disposal", async () => {
	...
	const done = { indices: [], status: null, done: true };
	...
	cancel();
	const cancelOther = pingInstanceServers(
		"B",
		[{ index: 0, address: "localhost" }],
		mock(),
		error,
...
test("edits and queries carry the instance and revision, including duplicate addresses", async () => {
	...
	expect(invoke.mock.calls[1]).toEqual([
		...
		{ instanceId: "A", revision: "original-revision", action },
		undefined,
	]);
});
...
test("server target survives Java installation and a normal launch clears it", () => {
	const result = Bun.spawnSync([
		process.execPath,
		"--conditions=browser",
		resolve(import.meta.dir, "fixtures/serverLaunch.mjs"),
	]);
	expect(result.stderr.toString()).toBe("");
	expect(result.exitCode).toBe(0);
});

test("ping transmits only targets, while paged icons keep file revision checks", async () => {
	const targets = [{ index: 50, address: "localhost" }];
	const cancel = pingInstanceServers("A", targets, mock(), mock());
	expect(starts[0].args.targets).toEqual(targets);
	expect(starts[0].args.revision).toBeUndefined();
	starts[0].resolve("native-request");
	await getInstanceServerIcons("A", "file-revision", [50]);
	expect(
		invoke.mock.calls.find(
			([command]) => command === "get_instance_server_icons",
		)[1],
	).toEqual({ instanceId: "A", revision: "file-revision", indices: [50] });
	cancel();
	await settle();
});
