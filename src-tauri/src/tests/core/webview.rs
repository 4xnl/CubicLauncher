...
fn environment_options_follow_main_without_replacing_default_arguments() {
    ...
    let window =
        secondary_window_config(&config, "log-test", WebviewUrl::App("/?log=test".into())).unwrap();
...
fn missing_main_configuration_is_an_error() {
    ...
    assert!(
        secondary_window_config(&config, "log-test", WebviewUrl::App("index.html".into())).is_err()
    );
}
