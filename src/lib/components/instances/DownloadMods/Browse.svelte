<script lang="ts">
	...
	let {
		...
		query = $bindable(""),
		allHits = [] as (ModrinthProject | CurseForgeProject)[],
		totalHits = 0,
		searching = false,
		loadingMore = false,
		...
		versionDropdownOptions = [] as {
			value: string;
			label: string;
			subtitle?: string;
		}[],
		gameVersion = "",
		installedModNames = new Set<string>(),
		onSearchInput,
		performSearch,
		handleNearEnd,
		switchSource,
		...
		source: ModSource;
		query: string;
		allHits: (ModrinthProject | CurseForgeProject)[];
		totalHits: number;
		searching: boolean;
		loadingMore: boolean;
		basket: SvelteMap<string, ModrinthProject | CurseForgeProject>;
		selectedMod: ModrinthProject | CurseForgeProject | null;
		selectedModVersions: (ModrinthVersion | CurseForgeFile)[];
	...
	function isModCompatible(
		...
		if ("versions" in project) {
			return project.versions.some((v: string) => v === gameVersion);
		}
		if ("latestFilesIndexes" in project) {
			return (
				project.latestFilesIndexes?.some(
					(idx: { gameVersion: string }) =>
						getGameVersion(idx.gameVersion) === gameVersion,
				) ?? false
			);
		}
		return true;
	...
	<main class="dm-main">
		...
		{#if totalHits > 0 && !searching}
			<div class="dm-results-meta">
				<span
					>{t("instanceView.downloadMods.resultsFound", {
						count: totalHits.toLocaleString(),
					})}</span
				>
			</div>
		{/if}
							...
							>
								<div class="dm-mod-icon-v">
									{#if getProjectIcon(project)}
										...
										/>
									{:else}
										<span class="dm-mod-icon-placeholder"
											>📦</span
										>
									{/if}
								</div>
...
{#if basket.size > 0}
	<div class="dm-bottom-bar">
		...
		<button type="button" class="dm-primary-btn" onclick={startReview}>
